...
Component* Potentiometer::construct( std::string type, std::string id )
...
Potentiometer::Potentiometer( std::string type, std::string id )
             : Dialed( type, id )
             , eElement( (id+"-eElement") )
             , m_pinA(  id+"-PinA", 0, this )
             , m_pinM(  id+"-PinM", 0, this )
             , m_pinB(   id+"-PinB", 0, this )
             , m_ePinA( id+"-ePinA", 1 )
             , m_ePinB( id+"-ePinB", 1 )
             , m_resA(  id+"-resA" )
             , m_resB(  id+"-resB" )
{
    ...
    addPropGroup( {"Main", {
new doubleProp<Potentiometer>( "Resistance", "Resistance"   ,"Ω", this, &Potentiometer::getRes, &Potentiometer::setRes ),
new doubleProp<Potentiometer>( "Value_Ohm" , "Current Value","Ω", this, &Potentiometer::getVal, &Potentiometer::setVal ),
    },0 } );
...
void Potentiometer::updateStep()
{
    ...
    double res1 = double(m_resist * m_dialWvalue / 1000);
...
double Potentiometer::getVal() { return m_resist* m_dialWvalue /1000; }
...
void Potentiometer::setVal( double val )
{
    ...
    m_dialWvalue=val*1000/m_resist ;
