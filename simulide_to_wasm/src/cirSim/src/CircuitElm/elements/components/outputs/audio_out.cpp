...
Component* AudioOut::construct( std::string type, std::string id )
...
AudioOut::AudioOut( std::string type, std::string id )
        : Component( type, id )
        , eResistor( id )
{
    ...
    addPropGroup( { "Main", {
new boolProp<AudioOut>( "Buzzer"   , "Buzzer"   ,"" , this, &AudioOut::buzzer, &AudioOut::setBuzzer ),
new doubleProp<AudioOut>( "Impedance", "Impedance","Ω", this, &AudioOut::res,    &AudioOut::setResSafe )
    },0} );
}
AudioOut::~AudioOut()
{
