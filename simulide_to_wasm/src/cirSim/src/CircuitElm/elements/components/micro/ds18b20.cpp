...
//注意idlable是没有设置的，后面的警告需要一个标志代表是哪个温度传感器

Component* Ds18b20::construct( std::string type, std::string id )
...
Ds18b20::Ds18b20( std::string type, std::string id )
       : Component( type, id )
       , eElement( id )
{
    //m_graphical = true;
    ...
    m_pin.resize(1);
    m_pin[0] = m_inpin = new IoPin(id+"-inPin", 0, this, openCo );
    ...
    addPropGroup( { "Main", {
new stringProp <Ds18b20>("ROM"    ,"ROM"                 ,""  , this, &Ds18b20::getROM,  &Ds18b20::setROM, propNoCopy ),
new doubleProp<Ds18b20>("Temp"   , "Temperature"    ,"°C", this, &Ds18b20::temp   , &Ds18b20::setTemp    ),
new doubleProp<Ds18b20>("TempInc", "Temp. increment","°C", this, &Ds18b20::tempInc, &Ds18b20::setTempInc ),
    },0} );
}
Ds18b20::~Ds18b20(){}
...
void Ds18b20::dataReceived() // Complete data has been received (it's in m_rxReg)
{
    switch( m_state )
    {
        ...
        default: std::cout << "Ds18b20::dataReceived  :  ERROR" << std::endl;
    }
}
...
void Ds18b20::romCommand( uint8_t cmd )
{
    switch( cmd )
    {
        ...
        default:{
            m_state = W1_IDLE;
            std::cout << "Ds18b20::command : Warning: ROM command Not implemented" << std::endl;
        }
    }
    m_lastCommand = cmd;
}
...
bool Ds18b20::bitROM( uint32_t bitIndex )
{
...
void Ds18b20::funCommand( uint8_t cmd )
{
    switch( cmd ){
        ...
        {  
            m_state = W1_IDLE;
            std::cout << "Ds18b20::command : Warning: Function command Not implemented" << std::endl;
    }   }
    m_lastCommand = cmd;
}
...
void Ds18b20::setROM( std::string ROMstr )
{
   /* if( Circuit::self()->pasting() ) return;*/

    bool ok;
    std::istringstream iss(ROMstr);
    //提取ROMstr字符串转化为流，逐个插入到lstROM中
    std::vector<std::string> lstROM
    {
        std::istream_iterator<std::string>{iss},
        std::istream_iterator<std::string>{}
    };

    for (size_t i = 0; i < 8 && i < lstROM.size(); ++i) 
    {
        try 
        {
            m_ROM[i] = std::stoi(lstROM[i], nullptr, 16);
            ok = true;
        }
        ...
        {
            // 处理转换错误
            std::cerr << "Conversion error at index " << i << " for value: " << lstROM[i] << std::endl;
        }
    }
    //std::vector<std::string> lstROM = ROMstr.split(" ");
    //lstROM.removeAll("");
...
std::string Ds18b20::arrayToHex( uint8_t* data, uint32_t len ) // Static
{
    std::string hexValue;
    for( uint32_t i=0; i<len; i++) hexValue.append( val2hex( data[i] ) ).append(" ");
    return hexValue;
}
