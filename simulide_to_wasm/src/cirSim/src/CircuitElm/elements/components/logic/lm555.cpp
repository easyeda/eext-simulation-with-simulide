...
Component* Lm555::construct( std::string type, std::string id )
...
Lm555::Lm555( std::string type, std::string id )
     : Component( type, id )
     , eElement( id )
     , m_ePinA( (id+"-ePinA"), 0 )
     , m_ePinB( (id+"-ePinB"), 0 )
     ...
     , m_resD(  (id+"-resD") )
{
    ...
    m_Gnd = new Pin(id+"-ePin0", 0, this );
    //m_Gnd->setLabelText( "Gnd" );
    m_pin[0] = m_Gnd;

    m_trigger = new Pin(id+"-ePin1", 1, this );
    //m_trigger->setLabelText( "Trg" );
    m_pin[1] = m_trigger;

    m_output = new IoPin(id+"-ePin2", 2, this, output );
    ...
    m_Reset = new Pin(  id+"-ePin3", 3, this );
    //m_Reset->setLabelText( "Rst" );
    m_pin[3] = m_Reset;

    m_cv = new Pin( id+"-ePin4", 4, this );
    //m_cv->setLabelText( "CV" );
    m_pin[4] = m_cv;

    m_threshold = new Pin( id+"-ePin5", 5, this );
    //m_threshold->setLabelText( "Thr" );
    m_pin[5] = m_threshold;

    m_discharge = new Pin( id+"-ePin6", 6, this );
    //m_discharge->setLabelText( "Dis" );
    m_pin[6] = m_discharge;

    m_Vcc = new Pin( id+"-ePin7", 7, this );
...
void Lm555::voltChanged()
{
    ...
    if( std::fabs(voltNeg-m_voltNeg) > 1e-3 )
    {
        m_output->setOutLowV( voltNeg );
        m_voltNeg = voltNeg;
        changed = true;
    }
    if( std::fabs(voltPos-m_voltPos) > 1e-3 )
    {
