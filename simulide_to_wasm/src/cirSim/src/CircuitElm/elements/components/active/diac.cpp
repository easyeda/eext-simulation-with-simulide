...
Component* Diac::construct( std::string type, std::string id )
...
Diac::Diac( std::string type, std::string id )
     : Component( type, id )
     , eElement( id )
{
    ...
    addPropGroup( { "Main", {
        new doubleProp<Diac>( "ResOn"   , "On Resistance"    ,"Ω", this, &Diac::resOn ,   &Diac::setResOn ),
        new doubleProp<Diac>( "ResOff"  , "Off Resistance"   ,"Ω", this, &Diac::resOff,   &Diac::setResOff ),
        new doubleProp<Diac>( "BrkVolt" , "Breakdown Voltage","V", this, &Diac::brkVolt,  &Diac::setBrkVolt ),
        new doubleProp<Diac>( "HoldCurr", "Hold Current"     ,"A", this, &Diac::holdCurr, &Diac::setHoldCurr )
    },0} );
...
void Diac::voltChanged()
{
    ...
    if( std::fabs(current) < m_holdCurr ) state = false; /// Mingw needs std::fabs
    if( std::fabs(voltage) > m_brkVolt  ) state = true;

    if( m_state == state ) return;
    Simulator::self()->notCorverged();

    //std::cout << "Diac::voltChanged" << state << std::endl;
    m_state = state;
