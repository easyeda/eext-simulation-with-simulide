...
const std::string LogicComponent::m_triggerList = 
    "None,Clock,Enable;"
    "None,Clock,Enable";  


LogicComponent::LogicComponent( std::string type, std::string id )
              : IoComponent( type, id )
              , eClockedDevice( id )
{
    m_oePin     = nullptr;
...
void LogicComponent::setTriggerStr( std::string t )
{
    m_triggerStr = t;
...
{
    for( uint32_t i=0; i<m_outPin.size(); ++i ) m_outPin[i]->setStateZ( !en );
}
