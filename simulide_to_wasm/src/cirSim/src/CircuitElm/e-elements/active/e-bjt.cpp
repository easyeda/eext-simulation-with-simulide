    ...
    , m_BCjunction( id+"BCjunct")
{
    ...
    m_vt = 0.025865;
    m_satCur = 1e-13;
    m_vCrit = m_vt * std::log(m_vt / (std::sqrt(2) * m_satCur));
...
void eBJT::voltChanged()
{
    ...
    double expBC = std::exp( voltBC*pcoef );
    double expBE = std::exp( voltBE*pcoef );
...
{
    if( vnew > m_vCrit && std::fabs(vnew-vold) > (2*m_vt) ){
        if( vold > 0 ){
            double arg = 1+(vnew-vold)/m_vt;
            if( arg > 0 )  vnew = vold + m_vt*std::log( arg );
            else           vnew = m_vCrit;
        }
        else vnew = m_vt*std::log( vnew/m_vt );
    }
    return vnew;
}
...
{
    m_vCrit = vCrit;
    m_satCur = m_vt/(std::exp( vCrit/m_vt )* std::sqrt(2));
    m_changed = true;
}
