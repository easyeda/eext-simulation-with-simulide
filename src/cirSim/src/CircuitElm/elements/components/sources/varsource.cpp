...
VarSource::VarSource( std::string type, std::string id )
         : Component( type, id )
         , eElement( id )
{
...
//    
//    if( checked )
//          msg = std::string::number( m_outValue,'f', 2 ).left(5)+" "+m_unit;
//    else  msg = std::string("---"+m_unit);
//        
...
bool VarSource::running() { return true; }     //---------------------------------------------------------控制是否运行，后续可添加代码
void VarSource::setRunning(bool r) { ; }  //后续可补充额外设置条件